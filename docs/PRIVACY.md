...
| **SMS** (symptoms, optional name, last known well, location link, risk %, "Demo message") | Backend `services/twilio_service.py` | **Twilio** -> `DEMO_PHONE_NUMBER` only. Logs record length and error code, never the text | **Twilio keeps the message record and body** (see section 3); the phone keeps the SMS | Demo phone owner, Twilio, carriers |
