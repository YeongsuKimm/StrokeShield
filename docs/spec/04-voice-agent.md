...
4. Face (patient close): "Please look at the camera and keep a relaxed, resting face." → `start_face_test`. The agent must NOT ask for the smile until the app says the resting-face capture is complete (`lib/agent/faceCues.ts` sends that cue when the capture goes neutral → smile, once per pass, including after an automatic retry); then "Now smile as wide as you can and hold it."
