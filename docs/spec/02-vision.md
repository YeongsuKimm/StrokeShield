...
**Instruction card (face and eyes):** each of these checks first shows its instruction alone, centred in the camera box, for `CAPTURE_TIMING.introMs` (4 s), with a "Starting in N…" count. Phase `intro`: nothing is measured, the framing gate is not evaluated and its wait-timeout has not started. It is shown once per check per session, so an automatic or manual retry goes straight back in. Arms has no card (its 3-2-1 cue plays that role).
