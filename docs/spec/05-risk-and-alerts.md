...
5. Else deliver **one text only** through `ALERT_CHANNEL` (default `email_sms`). An unknown/misspelled value is refused rather than silently selecting a provider. `email_sms` mails the carrier text gateway (`<10 digits>@SMS_GATEWAY_DOMAIN`, default Verizon `vtext.com`, address derived server-side from `DEMO_PHONE_NUMBER`) over Gmail SMTP (`SMTP_USER` + app password, STARTTLS 587, 10 s timeout) with a single-segment body (<= 160 chars, location rounded to 4 decimals, no patient name, control characters flattened so client text can never reach a header); `twilio` is the legacy path below. Same guards on both: env-only destination, `DRY_RUN`, one alert per 2 minutes, generic errors. Email-to-SMS is best-effort with no delivery receipt (Verizon's gateway ends 2027-03-31; AT&T and T-Mobile already shut theirs). Twilio: The app does not create an automated voice call and also excludes the optional contract `patient.name`. Free-text fields are clipped to 120 chars. Twilio failures return `{ok:false, error:"SMS could not be sent (Twilio error <code>)"}`: the raw Twilio message (which can echo numbers/SID) is logged type+code only and never returned.
...
- [ ] `?preflight=1` is green/amber as expected, then both live agent languages and one consented text are tested separately; preflight verifies configuration, not provider delivery.
