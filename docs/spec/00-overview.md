...
| Mobile | **Portrait for the whole check, Safari + Chrome only.** Phones get a 3:4 camera stream; the arm check quotes no distance and relies on the framing gate | Portrait is how people hold a phone, and a sideways step mid-check is one more thing to go wrong. 3:4 is the widest portrait view a phone gives, which the arm span needs. Other browsers are warned, never blocked |
