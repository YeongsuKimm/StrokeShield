...
| Calibration tooling (`?record=1` recorder, `pnpm calibrate` replay) | majesticcoder14 | done | Records the exact analyzer inputs of live runs to JSON; replays them offline against the severity anchors and prints a per-scenario table with false alarms / misses / wrong sides. **Open: everyone records ~20 runs (see `docs/CALIBRATION.md`), then tune thresholds** |
