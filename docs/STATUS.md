...
| Speech: core analysis (DSP features, scoring, QC) | open | done (synthetic only) | `models/audio.py` etc. 161 backend tests. Ramps/weights UNCALIBRATED; **no real human recordings yet**: record clips (docs/CALIBRATION.md) and run `python -m models.calibrate` |
| Speech: browser recorder + panels (`lib/speech/*`) | open | done (untested live) | 16 kHz WAV capture, auto-stop, `speechRunner.runSpeech()`, `?record=1` speech scenarios. Never run on a real mic: see the manual checklist in the recorder report / GETTING-STARTED |
