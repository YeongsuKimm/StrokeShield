...
def test_dry_run_sends_nothing():
    ...
    assert res["ok"] is True and res["dryRun"] is True and res["smsSid"] is None
...
def test_live_sms_uses_only_env_number_and_rate_limits(monkeypatch):
    ...
    assert first == {"ok": True, "dryRun": False, "smsSid": "SM1", "error": None}
    assert [kind for kind, _ in calls] == ["sms"]
    assert {kw["to"] for _, kw in calls} == {"+15555550100"}
    assert "maps.google.com/?q=39.33,-76.62" in calls[0][1]["body"]
