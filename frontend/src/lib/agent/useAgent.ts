...
export function useAgent() {
	...
	useEffect(() => {
		const unregister = registerAgentContextualUpdate(conversation.sendContextualUpdate)
		...
		if (conversation.status === 'connected') promptForPhase(useSession.getState().phase)
		return () => {
			unregister()
			unsubscribe()
		}
	}, [conversation.sendContextualUpdate, conversation.sendUserMessage, conversation.status])
