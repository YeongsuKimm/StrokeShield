...
export function failureFromError(e: unknown): AlertResponse {
  const err = e as { name?: string; status?: unknown; retryAfterS?: unknown; message?: unknown } | null
  ...
  if (err?.name === 'AbortError') return { ok: false, dryRun: false, error: 'network: the server did not answer in time' }
